...
const LoginHiringTeam = () => {
  ...
  const [userRole, setUserRole] = useState("hiring");
  ...
  useEffect(() => {
    const verifyAuth = async () => {
      const token = localStorage.getItem("HiringTeamToken");
      if (!token) return;
      ...
      try {
        ...
        localStorage.removeItem("HiringTeamToken",err);
      }
    };
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const response = await axios.post("/api/forgot-password/hiring", {
        email: resetEmail
      });
...
export default LoginHiringTeam;